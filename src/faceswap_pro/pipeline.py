...
from .observability import profile_event, profile_span
...
def run_pipeline(
    ...
    *,
    ...
    with profile_span("pipeline.validate_inputs"):
    ...
    with profile_span("pipeline.initialize_models", model_path=str(model_path)):
        models = model_bundle or initialize_models(config, model_path)
    profile_event(
        "pipeline_selected",
        backend=models.backend,
        execution_scope="video" if isinstance(models, VideoModelBundle) else "frame",
    )
    if isinstance(models, VideoModelBundle):
        with profile_span("pipeline.video.total", backend=models.backend):
    ...
    with profile_span("pipeline.frame.total", backend=models.backend):
...
def _run_video_pipeline(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="faceswap_pro_video_") as temp:
        ...
        with profile_span(
            "video.prepare_source_references",
        ...
        with profile_span("video.load_target_embedding"):
        ...
        with profile_span("video.mux_original_audio"):
            mux_original_audio(
                silent_video,
                input_video,
                output_video,
        ...
        with profile_span("video.write_visual_comparison"):
    ...
    with profile_span("video.probe_output"):
    ...
    with profile_span("video.write_quality_report"):
    ...
    with profile_span("video.write_manifest"):
...
def _prepare_source_references(
    *,
    ...
    with profile_span("video.source_identity_bank"):
    ...
    for index, sample in enumerate(samples):
        with profile_span("video.reference.align", reference_index=index):
            crop, _ = align_face(
                sample.image,
                sample.face.kps,
                512,
        ...
        with profile_span("video.reference.write", reference_index=index):
            if not cv2.imwrite(str(destination), crop):
                raise RuntimeError(
                    f"No se pudo escribir la referencia DreamID-V: {destination}"
                )
        references.append(
            VideoReference(
                path=destination,
...
def _run_frame_pipeline(
    *,
    ...
    analyzer = models.analyzer
    with profile_span("frame.build_source_identity"):
    ...
    with profile_span("frame.load_target_embedding"):
        target_embedding = load_reference_embedding(
            analyzer,
            target_reference,
            config.identity.source_min_score,
        )
    with profile_span("frame.initialize_processing_components"):
    ...
    with profile_span("frame.open_video_reader"):
    ...
    with tempfile.TemporaryDirectory(prefix="faceswap_pro_") as temp_dir:
        ...
        try:
            with profile_span("frame.open_video_writer"):
            ...
            raise
        try:
            with profile_span("frame.parallel_pipeline"):
            ...
            with profile_span("frame.close_video_writer"):
                writer.close()
        with profile_span("frame.mux_original_audio"):
            mux_original_audio(
                silent_path,
                input_video,
                output_video,
    ...
    with profile_span("frame.write_manifest"):
        manifest = _write_pipeline_manifest(
            output_video=output_video,
            manifest_dir=manifest_dir,
            input_video=input_video,
